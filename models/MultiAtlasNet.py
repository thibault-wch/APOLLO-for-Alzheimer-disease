...
class MultiAtlasNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.sparseMoE = SparseAtlasMoE(input_dim=dim_latent*2,
                                        num_atlas=num_atlas,
                                        num_heads=num_heads,
        ...
        self.layer2 = self._make_layer(
            block, dim_latent, layers[1], shortcut_type, stride=2)

        self.init_ln=nn.LayerNorm(dim_latent)

        # pet_extractor
        self.inplanes = 48
        self.pconv1 = nn.Conv3d(
            1,
            self.inplanes,
            kernel_size=7,
            stride=(2, 2, 2),
        ...
        self.player2 = self._make_layer(
            block, dim_latent, layers[1], shortcut_type, stride=2)
        self.pinit_ln = nn.LayerNorm(dim_latent)


        approx_gelu = lambda: nn.GELU()
        self.fc = Mlp(in_features=dim_latent*4,
    ...
    def forward(self, x, px, atlas):
        ...
        x = self.layer2(x)
        x = self.init_ln(
            (atlas.flatten(2).matmul(x.flatten(2).permute(0, 2, 1))) / (atlas.flatten(2).sum(2).unsqueeze(2)))
        ...
        px = self.player2(px)
        px = self.pinit_ln(
            (atlas.flatten(2).matmul(px.flatten(2).permute(0, 2, 1))) / (atlas.flatten(2).sum(2).unsqueeze(2)))
